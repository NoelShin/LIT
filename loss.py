...
class GatedUNetGenerator(BaseGenerator):
    def __init__(self, opt):
        ...
        fusion_layer = [[nn.Conv2d(2 * n_ch, n_ch, kernel_size=1, padding=0, stride=1)]]
        for i in range(n_downsample):
            ...
            if i != n_downsample - 1:
                gate_layer += [[ChannelAttentionLayer(2 * n_ch)]]
                fusion_layer += [[nn.Conv2d(4 * n_ch, 2 * n_ch, kernel_size=1, padding=0, stride=1)]]
    ...
    def forward(self, x):
        ...
        for i in range(self.n_downsample - 1, -1, -1):  # 3 2 1 0
            x = torch.cat([getattr(self, 'Up_layer_{}'.format(i))(x),
                           getattr(self, 'Gate_layer_{}'.format(i))(results[i])], dim=1)
            x = getattr(self, 'Fusion_layer_{}'.format(i))(x)
...
class PatchCritic(BaseNetwork):
    def __init__(self, opt):
        ...
        self.act = nn.LeakyReLU(0.2,
                                inplace=False)  # to avoid inplace activation. inplace activation cause error WGAN_GP
...
class ProgressiveGatedUNetGenerator(BaseGenerator):
    def __init__(self, opt):
        ...
        for i in range(n_downsample):
            ...
            if i != n_downsample - 1:
                gate_layer += [[ChannelAttentionLayer(2 * n_ch)]]
                fusion_layer += [[nn.Conv2d(4 * n_ch, 2 * n_ch, kernel_size=1, padding=0, stride=1, bias=True)]]
    ...
    def forward(self, x, level, level_in):
        ...
        for i in range(level):
            x = torch.cat(
                [getattr(self, 'Gate_layer_{}'.format(self.n_gate - 1 - i))(results[self.n_downsample - 1 - i]),
